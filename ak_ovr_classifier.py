...
def collect_ak_data(base_path='/Users/iinuma/Desktop/ダーモ'):
    """AK vs その他のデータ収集"""
    print("📁 AK OVRデータ収集中...")
    ...
    print(f"✅ 合計: {len(image_paths)}枚 (AK: {sum(labels)}, その他: {len(labels)-sum(labels)})")
    return image_paths, labels

def train_ak_ovr(output_dir='/Users/iinuma/Desktop/ダーモ/ak_ovr_weights', n_folds=3, epochs=5):
    """AK OVR分類器の学習"""
    print("🚀 AK One-vs-Rest分類器学習開始")
    print("=" * 60)
    
    # データ収集
    image_paths, labels = collect_ak_data()
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        print(f"  訓練: {len(X_train)}枚 (AK: {sum(y_train)})")
        print(f"  検証: {len(X_val)}枚 (AK: {sum(y_val)})")
        
        # データセット作成
        train_dataset = AKDataset(X_train, y_train, is_training=True)
        val_dataset = AKDataset(X_val, y_val, is_training=False)
        ...
        # モデル作成
        model = AKNet().to(device)
        
        # FocalLossで少数クラス(AK)を強調
        alpha = torch.tensor([0.3, 0.7])  # AK側を重視
        ...
        for epoch in range(epochs):
            ...
            if auc > best_auc:
                ...
                torch.save({
                    ...
                    'fold': fold
                }, f"{output_dir}/ak_ovr_fold{fold}.pth")
...
def predict_ak_prob(image_paths, weights_dir='/Users/iinuma/Desktop/ダーモ/ak_ovr_weights'):
    """AK確率予測（アンサンブル）"""
    models = []
    
    # 全foldモデル読み込み
    for fold in range(3):
        model_path = f"{weights_dir}/ak_ovr_fold{fold}.pth"
